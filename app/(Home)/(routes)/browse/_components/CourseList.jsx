  ...
  return (
    <div className='mt-5 grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-5'>
      {courses.map((course, index) => (
        <div 
          key={index}
          className='border rounder-lg p-2 cursor-pointer hover:border-purple-300'
        >
          <Image 
          ...
          />
          <div className='mt-2'>
            <h2 className='text-[18px] md:text-[18px] font-medium'>{course.name}</h2>
            <h2 className='test-gray-400 text-[13px]'>{course.createdBy.name}</h2>
          </div>
